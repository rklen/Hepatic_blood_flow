...
for(i in 1:length(studyNumbers)){
  filepath<-paste('C:/Users/Oona/Documents/Tpc/dit/array1_',studyNumbers[i],'.csv',sep='')
...
write.csv(df1,file=paste('dit_model1.csv',sep=''),row.names=F)
...
for(i in 1:length(studyNumbers)){
  ...
  for(l in 1:length(times)){
    input[l]<-linearInter(times[l],t,df[1,])
    organCurve[l]<-linearInter(times[l],t,df[4,])
  }
  #plot(times,input,type='l')
  plot(times,organCurve,type='l',ylab=i)
  initialValues<-c(1,1)
...
fit_model<-function(param,input,t1,t2){
  fA<-abs(param[1])
  fp<-abs(param[2])
  k<-abs(param[3])
  E<-1/(1+exp(-param[4]))
  ...
  for(i in 2:length(input)){
    ...
    if(i-t1-t2-1<1){c0<-0}else{c0<-input[i-t1-t2-1]}
    if(i-t2-1<1){cpv1<-0}else{cpv1<-cPV[i-t2-1]}
    cT<-c(cT,fA*c0+E*fp*cpv1+(1-k)*cT[i-1])
  }
  v<-(1-Vb)*cT+Vb*(fA*input+E*fp*cPV)/(fA+E*fp)
  return(v)
}
errorfunction<-function(param,input,organCurve,t1,t2){
  return(sum((organCurve-fit_model(param,input,t1,t2))^2))
}
optimizationAlg<-function(initialValues,input,organCurve,t1,t2){
  tryCatch(
    {
      res<-nlm(errorfunction,initialValues,input=input,
               organCurve=organCurve,t1=t1,t2=t2,
...
for(i in 1:length(studyNumbers)){
  ...
  errorsFort<-c()
  for(t2 in ((0:6)*10)){
    param<-optimizationAlg(initialValues,input,organCurve,t1,t2)
    err<-errorfunction(param,input,organCurve,t1,t2)
    errorsFort<-c(errorsFort,err)
  }
  t2_l<-c((0:6)*10)[which.min(errorsFort)]
  errorsFort<-c()
  ...
  param<-optimizationAlg(initialValues,input,organCurve,t1,t2)
  points(times,fit_model(param,input,t1,t2),type='l')
  df1[i,1:length(param)]<-param
  df1[i,7]<-t2
  df1[i,9]<-meanRelError(fit_model(param,input,t1,t2),organCurve)
  err<-errorfunction(param,input/1000,organCurve/1000,t1,t2)
  df1[i,10]<-err/(length(times)-1)
  df1[i,11]<-280*log(df1[i,10])+2*7
  df1[i,12]<-1-err/sum((organCurve/1000-mean(organCurve/1000))^2)
  mod1<-cbind(times[-1],fit_model(param,input,t1,t2)[-1])[times=t,2]/1000
  oc1<-cbind(times[-1],organCurve[-1])[times=t,2]/1000
  df1[i,13]<-meanRelError(mod1,oc1)
  err<-sum((mod1-oc1)^2)
  df1[i,14]<-err/length(oc1)
  df1[i,15]<-length(oc1)*log(df1[i,12])+2*7
  df1[i,16]<-1-err/sum((oc1-mean(oc1))^2)
}
write.csv(df1,file=paste('dit_model2.csv',sep=''),row.names=F)
...
write.csv(df1,file=paste('dit_model3.csv',sep=''),row.names=F)
...
write.csv(df1,file=paste('dit_model4.csv',sep=''),row.names=F)

df1<-read.csv('C:/Users/Oona/Documents/Tpc/dit/dit_model3.csv')
y<-df1[,6]
y<-abs(df1[,2])*60*1/(1+exp(-df1[,4]))+abs(df1[,1])*60
y<-(abs(df1[,2])*1/(1+exp(-df1[,4]))+abs(df1[,1]))/abs(df1[,3])
y<-abs(df1[,2])*1/(1+exp(-df1[,4]))/(abs(df1[,2])*1/(1+exp(-df1[,4]))+abs(df1[,1]))
...
df1<-read.csv('C:/Users/Oona/Documents/Tpc/dit/dit_model4.csv')
...
cor(mendf[,8],mendf[,9])
cor.test(df[,8],df[,9])
cor.test(womendf[,8],womendf[,9])
cor.test(mendf[,8],mendf[,9])

cor(df[,10],as.numeric(df[,2]))
cor(womendf[,10],as.numeric(womendf[,2]))
cor(mendf[,10],as.numeric(mendf[,2]))
cor.test(df[,10],as.numeric(df[,2]))
cor.test(womendf[,10],as.numeric(womendf[,2]))
cor.test(mendf[,10],as.numeric(mendf[,2]))
...
cor.test(df[!is.na(df[,4]),10],df[!is.na(df[,4]),4])
cor.test(womendf[!is.na(womendf[,4]),10],womendf[!is.na(womendf[,4]),4])
cor.test(mendf[!is.na(mendf[,4]),10],mendf[!is.na(mendf[,4]),4])
...
cor.test(df[!is.na(df[,7]),10],df[!is.na(df[,7]),7])
cor.test(womendf[!is.na(womendf[,7]),10],womendf[!is.na(womendf[,7]),7])
cor.test(mendf[!is.na(mendf[,7]),10],mendf[!is.na(mendf[,7]),7])
...
plot(as.numeric(mendf[,2]),mendf[,10],xlim=c(40,85),ylim=c(0,3),
     lwd=2,ylab='Total HBF (mL/min/mL)',xlab='Age (years)',
     cex.axis=1.3,cex.lab=1.3,col='blue')
...
plot(mendf[!is.na(mendf[,4]),4],mendf[!is.na(mendf[,4]),10],
     ...
     lwd=2,ylab='Total HBF (mL/min/mL)',xlab='Weight (kg)',
     cex.axis=1.3,cex.lab=1.3,col='blue')
...
filepath<-paste('C:/Users/Oona/Documents/Tpc/dit/array1_',studyNumbers[1],'.csv',sep='')
df<-read.csv(filepath)
df<-as.matrix(df)
colnames(df)<-NULL
rownames(df)<-NULL
...
returnPV<-function(param,input,t1){
  fA<-abs(param[1])
  fp<-abs(param[2])
  k<-abs(param[3])
  E<-1/(1+exp(-param[4]))
  ...
  return(as.vector(unlist(cPV)))
}
returnSpleenPV<-function(param,input,t1,t2){
  fA<-abs(param[1])
  fp<-abs(param[2])
  k<-abs(param[3])
  E<-1/(1+exp(-param[4]))
...
dfInput0<-matrix(data=NA,nrow=length(studyNumbers),ncol=281)
dfInput1<-matrix(data=NA,nrow=length(studyNumbers),ncol=281)
dfInput2<-matrix(data=NA,nrow=length(studyNumbers),ncol=281)
dfInput3<-matrix(data=NA,nrow=length(studyNumbers),ncol=281)
for(i in 1:length(studyNumbers)){
  filepath<-paste('C:/Users/Oona/Documents/Tpc/dit/array1_',studyNumbers[i],'.csv',sep='')
  df<-read.csv(filepath)
  df<-as.matrix(df)
  ...
  input3<-c()
  for(l in 1:length(times)){
    input0[l]<-linearInter(times[l],t,df[1,])
    input3[l]<-linearInter(times[l],t,df[6,])
  }
  dfInput0[i,]<-input0/1000
  dfInput3[i,]<-input3/1000
  df1<-read.csv('C:/Users/Oona/Documents/Tpc/dit/dit_model2.csv')
  param<-df1[i,1:5]
  t1<-df1[i,6]
  t2<-df1[i,7]
  cS<-returnSpleenPV(param,input0,t1,t2)
  dfInput1[i,]<-cS/1000
  df1<-read.csv('C:/Users/Oona/Documents/Tpc/dit/dit_model3.csv')
  param<-df1[i,1:5]
  t1<-df1[i,6]
  cPV<-returnPV(param,input0,t1)
  dfInput2[i,]<-cPV/1000
}

l0<-c(colMeans(dfInput0)-colSds(dfInput0),rev(colMeans(dfInput0)+colSds(dfInput0)))
l1<-c(colMeans(dfInput1)-colSds(dfInput1),rev(colMeans(dfInput1)+colSds(dfInput1)))
l2<-c(colMeans(dfInput2)-colSds(dfInput2),rev(colMeans(dfInput2)+colSds(dfInput2)))
l3<-c(colMeans(dfInput3)-colSds(dfInput3),rev(colMeans(dfInput3)+colSds(dfInput3)))

mycol0 <- rgb(0, 128, 255, max = 255, alpha = 100, names = "blue50")
mycol0new<-rgb(0, 128, 255, max = 255, alpha = 255, names = "blue50") 
mycol1 <- rgb(0, 0, 0, max = 255, alpha = 100, names = "blue50")
mycol1new <- rgb(0, 0, 0, max = 255, alpha = 255, names = "blue50")
mycol2 <- rgb(0, 128, 128, max = 255, alpha = 100, names = "blue50")
mycol2new <-rgb(0, 128, 128, max = 255, alpha = 255, names = "blue50")
mycol3 <- rgb(0, 0, 255, max = 255, alpha = 100, names = "blue50")
mycol3new <- 'blue'
plot(n,type='l',
     xlim=c(0,280),ylim=c(0,100),
     lwd=2,ylab='Activity concentration (kBq/mL)',xlab='Time (s)',
...
polygon(c(times,rev(times)),l0,col=mycol0,border=mycol0new)
polygon(c(times,rev(times)),l1,col=mycol1,border=mycol1new)
polygon(c(times,rev(times)),l2,col=mycol2,border=mycol2new)
polygon(c(times,rev(times)),l3,col=mycol3,border=mycol3new)
polygon(c(times,rev(times)),l0,border=mycol0new,lwd=2,lty=2)
polygon(c(times,rev(times)),l1,border=mycol1new,lwd=2,lty=2)
polygon(c(times,rev(times)),l2,border=mycol2new,lwd=2,lty=2)
polygon(c(times,rev(times)),l3,border=mycol3new,lwd=2,lty=2)
points(times,colMeans(dfInput0),type='l',lwd=2,col=mycol0new)
points(times,colMeans(dfInput1),type='l',lwd=2,col=mycol1new)
points(times,colMeans(dfInput2),type='l',lwd=2,col=mycol2new)
points(times,colMeans(dfInput3),type='l',lwd=2,col=mycol3new)

legend('topright',legend=c('Aorta',
                           'PV (fitted with spleen)',
                           'PV (fitted without spleen)',
                           'PV (measured)'),
       lty=c(1,1,1,1),
       col=c(mycol0new,mycol1new,mycol2new,mycol3new),
       lwd=c(2,2,2,2),cex=1.4)
